...
export default function Analysis() {
  ...
  const handleTextSubmit = () => {
    ...
    setTimeout(() => {
      ...
      const hasOffensiveWords = text.toLowerCase().includes('idiot') || 
                               text.toLowerCase().includes('stupid') || 
                               text.toLowerCase().includes('hate');
      
      const hasProfanity = text.toLowerCase().includes('fuck') || 
                          text.toLowerCase().includes('shit') || 
                          text.toLowerCase().includes('damn');
      
      const hasViolence = text.toLowerCase().includes('kill') || 
                         text.toLowerCase().includes('die') || 
                         text.toLowerCase().includes('hurt');
      
      const hasSpam = text.toLowerCase().includes('buy now') || 
      ...
      const mockResult: ModerationResult = {
        ...
        flags,
        summary: anyFlagged 
          ? `Content ${hasProfanity ? 'contains profanity' : ''} ${hasOffensiveWords ? (hasProfanity ? 'and' : '') + ' may be offensive or toxic' : ''} ${hasViolence ? (hasProfanity || hasOffensiveWords ? 'and' : '') + ' contains violent language' : ''} ${hasSpam ? (hasProfanity || hasOffensiveWords || hasViolence ? 'and' : '') + ' appears to be spam' : ''}.`
      ...
      <main className="flex-grow bg-muted/30 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">Test our moderation API with your content</p>
          </div>
              ...
              <CardContent>
                <Tabs defaultValue="text" className="space-y-4">
                  ...
                  <TabsContent value="text" className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <Label htmlFor="threshold">Sensitivity Threshold: {threshold}</Label>
                      </div>
                      <Slider
                        id="threshold"
                        min={0.1}
                  ...
                  <TabsContent value="image" className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <Label htmlFor="image-threshold">Sensitivity Threshold: {threshold}</Label>
                      </div>
                      <Slider
                        id="image-threshold"
                        min={0.1}
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <h3 className="text-sm font-medium mb-3">Category Analysis:</h3>
                          ...
                          <div key={index} className="space-y-1">
                            <div className="flex justify-between">
                              ...
                              <span className={`text-sm font-medium ${
                                flag.flagged ? 'text-destructive' : 'text-muted-foreground'
                              }`}>
                    ...
                    <div className="border-t border-border pt-4">
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          ...
                          <span className="ml-1 font-mono">{result.requestId}</span>
                        </div>
                        <div>
                  ...
                  <div className="h-[300px] flex items-center justify-center text-center">
                    <div>
                      <p className="text-muted-foreground mb-2">No content analyzed yet</p>
                      <p className="text-sm text-muted-foreground">
